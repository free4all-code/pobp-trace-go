...
func newProfiler(opts ...Option) (*profiler, error) {
	...
	if os.Getenv("POBP_PROFILING_WAIT_PROFILE") != "" {
	...
	if cfg.agentless {
		if !isAPIKeyValid(cfg.apiKey) {
			return nil, errors.New("profiler.WithAgentlessUpload requires a valid API key. Use profiler.WithAPIKey or the POBP_API_KEY env variable to set it")
		}
		...
		// misconfiguration.
		if cfg.apiKey != "" {
			log.Warn("You are currently setting profiler.WithAPIKey or the POBP_API_KEY env variable, but as of dd-trace-go v1.30.0 this value is getting ignored by the profiler. Please see the profiler.WithAPIKey go docs and verify that your integration is still working. If you can't remove POBP_API_KEY from your environment, you can use WithAPIKey(\"\") to silence this warning.")
		}
