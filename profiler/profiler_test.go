...
func TestAllUploaded(t *testing.T) {
	...
	old, ok := os.LookupEnv("POBP_PROFILING_WAIT_PROFILE")
	os.Setenv("POBP_PROFILING_WAIT_PROFILE", "1")
	if ok {
		defer os.Setenv("POBP_PROFILING_WAIT_PROFILE", old)
	} else {
		defer os.Unsetenv("POBP_PROFILING_WAIT_PROFILE")
	}
	Start(
		WithAgentAddr(server.Listener.Addr().String()),
		WithProfileTypes(
