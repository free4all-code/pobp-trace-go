...
import (
	...
	"git.proto.group/protoobp/pobp-trace-go/internal/osinfo"
...
const (
	...
	// It can be overwritten using the POBP_PROFILING_UPLOAD_TIMEOUT env variable
...
func defaultConfig() (*config, error) {
	c := config{
		...
		deltaProfiles:     internal.BoolEnv("POBP_PROFILING_DELTA", true),
		logStartup:        true,
	}
	...
	if v := os.Getenv("POBP_PROFILING_UPLOAD_TIMEOUT"); v != "" {
		d, err := time.ParseDuration(v)
		if err != nil {
			return nil, fmt.Errorf("POBP_PROFILING_UPLOAD_TIMEOUT: %s", err)
		}
		WithUploadTimeout(d)(&c)
	}
	if v := os.Getenv("POBP_API_KEY"); v != "" {
		WithAPIKey(v)(&c)
	}
	if internal.BoolEnv("POBP_PROFILING_AGENTLESS", false) {
		WithAgentlessUpload()(&c)
	}
	if v := os.Getenv("POBP_SITE"); v != "" {
		WithSite(v)(&c)
	}
	...
	// not for public use
	if v := os.Getenv("POBP_PROFILING_URL"); v != "" {
		WithURL(v)(&c)
	}
	// not for public use
	if v := os.Getenv("POBP_PROFILING_OUTPUT_DIR"); v != "" {
		withOutputDir(v)(&c)
	}
	if v := os.Getenv("POBP_PROFILING_WAIT_PROFILE_MAX_GOROUTINES"); v != "" {
		n, err := strconv.Atoi(v)
		if err != nil {
			return nil, fmt.Errorf("POBP_PROFILING_WAIT_PROFILE_MAX_GOROUTINES: %s", err)
		}
...
// WithAPIKey sets the Datadog API Key and takes precedence over the POBP_API_KEY
...
// is true. This option takes precedence over the POBP_PROFILING_DELTA
...
// POBP_PROFILING_UPLOAD_TIMEOUT env variable. Using a negative value or 0 will
