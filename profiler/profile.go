...
import (
	...
	"git.proto.group/protoobp/pobp-trace-go/profiler/internal/extensions"
	"git.proto.group/protoobp/pobp-trace-go/profiler/internal/pprofutils"
...
const (
	...
	// available within DD by setting the POBP_PROFILING_WAIT_PROFILE env variable.
	...
	expGoroutineWaitProfile: {
		...
		Collect: func(p *profiler) ([]byte, error) {
			if n := runtime.NumGoroutine(); n > p.cfg.maxGoroutinesWait {
				return nil, fmt.Errorf("skipping goroutines wait profile: %d goroutines exceeds POBP_PROFILING_WAIT_PROFILE_MAX_GOROUTINES limit of %d", n, p.cfg.maxGoroutinesWait)
			}
