...
func (s *span) Context() pobptrace.SpanContext { return s.context }
...
func (s *span) Finish(opts ...pobptrace.FinishOption) {
	t := now()
	if len(opts) > 0 {
		cfg := pobptrace.FinishConfig{
			NoDebugStack: s.noDebugStack,
		}
		for _, fn := range opts {
			fn(&cfg)
...
func (s *span) Format(f fmt.State, c rune) {
	...
	case 'v':
		if svc := globalconfig.ServiceName(); svc != "" {
			fmt.Fprintf(f, "pobp.service=%s ", svc)
		}
		if tr, ok := internal.GetGlobalTracer().(*tracer); ok {
			if tr.config.env != "" {
				fmt.Fprintf(f, "pobp.env=%s ", tr.config.env)
			}
			if tr.config.version != "" {
				fmt.Fprintf(f, "pobp.version=%s ", tr.config.version)
			}
		} else {
			if env := os.Getenv("POBP_ENV"); env != "" {
				fmt.Fprintf(f, "pobp.env=%s ", env)
			}
			if v := os.Getenv("POBP_VERSION"); v != "" {
				fmt.Fprintf(f, "pobp.version=%s ", v)
			}
		}
		fmt.Fprintf(f, `pobp.trace_id="%d" pobp.span_id="%d"`, s.TraceID, s.SpanID)
	default:
		fmt.Fprintf(f, "%%!%c(pobptrace.Span=%v)", c, s)
	}
}

const (
	keySamplingPriority        = "_sampling_priority_v1"
	keySamplingPriorityRate    = "_pobp.agent_psr"
	keyDecisionMaker           = "_pobp.p.dm"
	keyServiceHash             = "_pobp.dm.service_hash"
	keyOrigin                  = "_pobp.origin"
	keyHostname                = "_pobp.hostname"
	keyRulesSamplerAppliedRate = "_pobp.rule_psr"
	keyRulesSamplerLimiterRate = "_pobp.limit_psr"
	keyMeasured                = "_pobp.measured"
	...
	keyTopLevel = "_pobp.top_level"
	// keyPropagationError holds any error from propagated trace tags (if any)
	keyPropagationError = "_pobp.propagation_error"
)
