...
import (
	...
	"git.proto.group/protoobp/pobp-trace-go/internal/traceprof"
...
var (
	...
	// defaultMaxTagsHeaderLen specifies the default maximum length of the X-ProtoOBP-Tags header value.
...
type config struct {
	...
	// hostname is automatically assigned when the POBP_TRACE_REPORT_HOSTNAME is set to true,
	...
	// combination of the environment variables POBP_AGENT_HOST and POBP_DOGSTATSD_PORT.
...
// maxPropagatedTagsLength limits the size of POBP_TRACE_X_DATADOG_TAGS_MAX_LENGTH to prevent HTTP 413 responses.
...
func newConfig(opts ...StartOption) *config {
	...
	if internal.BoolEnv("POBP_TRACE_ANALYTICS_ENABLED", false) {
		globalconfig.SetAnalyticsRate(1.0)
	}
	if os.Getenv("POBP_TRACE_REPORT_HOSTNAME") == "true" {
		var err error
	...
	if v := os.Getenv("POBP_TRACE_SOURCE_HOSTNAME"); v != "" {
		c.hostname = v
	}
	if v := os.Getenv("POBP_ENV"); v != "" {
		c.env = v
	}
	if v := os.Getenv("POBP_TRACE_FEATURES"); v != "" {
		WithFeatureFlags(strings.FieldsFunc(v, func(r rune) bool {
			return r == ',' || r == ' '
		})...)(c)
	}
	if v := os.Getenv("POBP_SERVICE"); v != "" {
		c.serviceName = v
		globalconfig.SetServiceName(v)
	}
	if ver := os.Getenv("POBP_VERSION"); ver != "" {
		c.version = ver
	}
	if v := os.Getenv("POBP_SERVICE_MAPPING"); v != "" {
		forEachStringTag(v, func(key, val string) { WithServiceMapping(key, val)(c) })
	}
	if v := os.Getenv("POBP_TAGS"); v != "" {
	...
	c.logStartup = internal.BoolEnv("POBP_TRACE_STARTUP_LOGS", true)
	c.runtimeMetrics = internal.BoolEnv("POBP_RUNTIME_METRICS_ENABLED", false)
	c.debug = internal.BoolEnv("POBP_TRACE_DEBUG", false)
	c.enabled = internal.BoolEnv("POBP_TRACE_ENABLED", true)
	...
	if c.propagator == nil {
		envKey := "POBP_TRACE_X_DATADOG_TAGS_MAX_LENGTH"
...
func defaultDogstatsdAddr() string {
	envHost, envPort := os.Getenv("POBP_AGENT_HOST"), os.Getenv("POBP_DOGSTATSD_PORT")
...
func WithLogger(logger pobptrace.Logger) StartOption {
	return func(c *config) {
		c.logger = logger
	}
}
...
func WithServiceName(name string) StartOption {
	return func(c *config) {
		...
		if globalconfig.ServiceName() != "" {
			log.Warn("pobptrace/tracer: deprecated config WithServiceName should not be used " +
				"with `WithService` or `POBP_SERVICE`; integration service name will not be set.")
		}
...
// The default value is the environment variable POBP_ENV, if it is set.
...
//   2. The host is determined by POBP_AGENT_HOST, and defaults to "localhost"
//   3. The port is retrieved from the agent. If not present, it is determined by POBP_DOGSTATSD_PORT, and defaults to 8125
...
// POBP_PROFILING_CODE_HOTSPOTS_COLLECTION_ENABLED env variable or true.
...
// the value of the POBP_PROFILING_ENDPOINT_COLLECTION_ENABLED env variable or
// true.
...
// more correct to refer to it as the type as the origin, pobptrace.StartSpanOption.
type StartSpanOption = pobptrace.StartSpanOption
...
// created span.
func ChildOf(ctx pobptrace.SpanContext) StartSpanOption {
	return func(cfg *pobptrace.StartSpanConfig) {
		cfg.Parent = ctx
	}
}
...
// to help godoc group all the functions returning it together. It is considered
// more correct to refer to it as the type as the origin, pobptrace.FinishOption.
type FinishOption = pobptrace.FinishOption

// FinishTime sets the given time as the finishing time for the span. By default,
// the current time is used.
func FinishTime(t time.Time) FinishOption {
	return func(cfg *pobptrace.FinishConfig) {
		cfg.FinishTime = t
	}
}
