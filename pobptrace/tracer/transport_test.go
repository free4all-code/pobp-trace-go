...
func TestResolveAgentAddr(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run("", func(t *testing.T) {
			if tt.envHost != "" {
				os.Setenv("POBP_AGENT_HOST", tt.envHost)
				defer os.Unsetenv("POBP_AGENT_HOST")
			}
			if tt.envPort != "" {
				os.Setenv("POBP_TRACE_AGENT_PORT", tt.envPort)
...
func TestTraceCountHeader(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		header := r.Header.Get("X-ProtoOBP-Trace-Count")
		assert.NotEqual("", header, "X-ProtoOBP-Trace-Count header should be here")
