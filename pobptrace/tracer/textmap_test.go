...
func TestTextMapPropagatorTraceTagsWithPriority(t *testing.T) {
	src := TextMapCarrier(map[string]string{
		...
		DefaultTraceIDHeader:  "1",
		DefaultParentIDHeader: "1",
		traceTagsHeader:       "hello=world,_pobp.p.dm=934086a6-4",
	})
	tracer := newTracer()
	ctx, err := tracer.Extract(src)
	assert.Nil(t, err)
	...
	assert.Equal(t, map[string]string{
		"hello":    "world",
		"_pobp.p.dm": "934086a6-4",
	}, sctx.trace.propagatingTags)
	dst := map[string]string{}
	err = tracer.Inject(child.Context(), TextMapCarrier(dst))
	assert.Nil(t, err)
	assert.Len(t, dst, 4)
	assert.Equal(t, strconv.Itoa(int(childSpanID)), dst["x-protoobp-parent-id"])
	assert.Equal(t, "1", dst["x-protoobp-trace-id"])
	assert.Equal(t, "1", dst["x-protoobp-sampling-priority"])
	assertTraceTags(t, "hello=world,_pobp.p.dm=934086a6-4", dst["x-protoobp-tags"])
}
...
func TestTextMapPropagator(t *testing.T) {
	t.Run("InvalidTraceTagsHeader", func(t *testing.T) {
		...
		sctx, ok := ctx.(*spanContext)
		assert.True(t, ok)
		assert.Equal(t, "decoding_error", sctx.trace.tags["_pobp.propagation_error"])
	})
	...
	t.Run("ExtractTraceTagsTooLong", func(t *testing.T) {
		...
		sctx, ok := ctx.(*spanContext)
		assert.True(t, ok)
		assert.Equal(t, "extract_max_size", sctx.trace.tags["_pobp.propagation_error"])
	})
	...
	t.Run("InjectTraceTagsTooLong", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, map[string]string{
			"x-protoobp-parent-id":         strconv.Itoa(int(childSpanID)),
			"x-protoobp-trace-id":          strconv.Itoa(int(childSpanID)),
			"x-protoobp-sampling-priority": "1",
		}, dst)
		assert.Equal(t, "inject_max_size", child.Context().(*spanContext).trace.tags["_pobp.propagation_error"])
	})
	...
	t.Run("InvalidTraceTags", func(t *testing.T) {
		...
		assert.Len(t, dst, 4)
		assert.Equal(t, strconv.Itoa(int(childSpanID)), dst["x-protoobp-parent-id"])
		assert.Equal(t, strconv.Itoa(int(childSpanID)), dst["x-protoobp-trace-id"])
		assert.Equal(t, "1", dst["x-protoobp-sampling-priority"])
		assertTraceTags(t, "_pobp.p.dm=-1,_dd.p.hello1=world", dst["x-protoobp-tags"])
		assert.Equal(t, "encoding_error", child.Context().(*spanContext).trace.tags["_pobp.propagation_error"])
	})
...
func TestB3(t *testing.T) {
	t.Run("inject", func(t *testing.T) {
		os.Setenv("POBP_PROPAGATION_STYLE_INJECT", "B3")
		defer os.Unsetenv("POBP_PROPAGATION_STYLE_INJECT")
	...
	t.Run("extract", func(t *testing.T) {
		os.Setenv("POBP_PROPAGATION_STYLE_EXTRACT", "b3")
		defer os.Unsetenv("POBP_PROPAGATION_STYLE_EXTRACT")
	...
	t.Run("config", func(t *testing.T) {
		os.Setenv("POBP_PROPAGATION_STYLE_INJECT", "datadog")
		defer os.Unsetenv("POBP_PROPAGATION_STYLE_INJECT")
