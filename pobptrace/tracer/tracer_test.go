...
import (
	...
	maininternal "git.proto.group/protoobp/pobp-trace-go/internal"
...
func TestMain(m *testing.M) {
	if maininternal.BoolEnv("POBP_APPSEC_ENABLED", false) {
...
func TestTracerCleanStop(t *testing.T) {
	...
	if old := os.Getenv("POBP_APPSEC_ENABLED"); old != "" {
		// avoid CI timeouts due to AppSec slowing down this test
		os.Unsetenv("POBP_APPSEC_ENABLED")
		defer os.Setenv("POBP_APPSEC_ENABLED", old)
	}
	os.Setenv("POBP_TRACE_STARTUP_LOGS", "0")
	defer os.Unsetenv("POBP_TRACE_STARTUP_LOGS")
...
func TestTracerRuntimeMetrics(t *testing.T) {
	...
	t.Run("env", func(t *testing.T) {
		os.Setenv("POBP_RUNTIME_METRICS_ENABLED", "true")
		defer os.Unsetenv("POBP_RUNTIME_METRICS_ENABLED")
		tp := new(testLogger)
	...
	t.Run("overrideEnv", func(t *testing.T) {
		os.Setenv("POBP_RUNTIME_METRICS_ENABLED", "false")
		defer os.Unsetenv("POBP_RUNTIME_METRICS_ENABLED")
		tp := new(testLogger)
		tracer := newTracer(WithRuntimeMetrics(), WithLogger(tp), WithDebugMode(true))
		defer tracer.Stop()
...
func TestTracerReportsHostname(t *testing.T) {
	...
	t.Run("POBP_TRACE_REPORT_HOSTNAME/set", func(t *testing.T) {
		os.Setenv("POBP_TRACE_REPORT_HOSTNAME", "true")
		defer os.Unsetenv("POBP_TRACE_REPORT_HOSTNAME")
	...
	t.Run("POBP_TRACE_REPORT_HOSTNAME/unset", func(t *testing.T) {
		tracer, _, _, stop := startTestTracer(t)
		defer stop()
	...
	t.Run("POBP_TRACE_SOURCE_HOSTNAME/set", func(t *testing.T) {
		os.Setenv("POBP_TRACE_SOURCE_HOSTNAME", "hostname-test")
	...
	t.Run("POBP_TRACE_SOURCE_HOSTNAME/unset", func(t *testing.T) {
		tracer, _, _, stop := startTestTracer(t)
		defer stop()
