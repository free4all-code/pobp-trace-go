...
import (
	...
	"git.proto.group/protoobp/pobp-trace-go/internal/samplernames"
)
...
const (
	headerPropagationStyleInject  = "POBP_PROPAGATION_STYLE_INJECT"
	headerPropagationStyleExtract = "POBP_PROPAGATION_STYLE_EXTRACT"
)
...
const (
	...
	DefaultPriorityHeader = "x-protoobp-sampling-priority"
)
...
const originHeader = "x-protoobp-origin"

// traceTagsHeader holds the propagated trace tags
const traceTagsHeader = "x-protoobp-tags"
...
func (p *chainedPropagator) Inject(spanCtx pobptrace.SpanContext, carrier interface{}) error {
...
func (p *chainedPropagator) Extract(carrier interface{}) (pobptrace.SpanContext, error) {
...
func (p *propagator) Inject(spanCtx pobptrace.SpanContext, carrier interface{}) error {
...
func (p *propagator) injectTextMap(spanCtx pobptrace.SpanContext, writer TextMapWriter) error {
...
func (p *propagator) Extract(carrier interface{}) (pobptrace.SpanContext, error) {
	switch c := carrier.(type) {
	case TextMapReader:
...
func (p *propagator) extractTextMap(reader TextMapReader) (pobptrace.SpanContext, error) {
	var ctx spanContext
...
func (p *propagatorB3) Inject(spanCtx pobptrace.SpanContext, carrier interface{}) error {
	switch c := carrier.(type) {
	case TextMapWriter:
		return p.injectTextMap(spanCtx, c)
	default:
...
func (*propagatorB3) injectTextMap(spanCtx pobptrace.SpanContext, writer TextMapWriter) error {
	ctx, ok := spanCtx.(*spanContext)
	if !ok || ctx.traceID == 0 || ctx.spanID == 0 {
		return ErrInvalidSpanContext
	}
...
func (p *propagatorB3) Extract(carrier interface{}) (pobptrace.SpanContext, error) {
	switch c := carrier.(type) {
	case TextMapReader:
		return p.extractTextMap(c)
	default:
...
func (*propagatorB3) extractTextMap(reader TextMapReader) (pobptrace.SpanContext, error) {
	var ctx spanContext
	err := reader.ForeachKey(func(k, v string) error {
		var err error
		key := strings.ToLower(k)
