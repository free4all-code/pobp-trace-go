...
func TestRuleEnvVars(t *testing.T) {
	t.Run("sample-rate", func(t *testing.T) {
		...
		for _, tt := range []struct {
			...
			os.Setenv("POBP_TRACE_SAMPLE_RATE", tt.in)
	...
	t.Run("rate-limit", func(t *testing.T) {
		...
		for _, tt := range []struct {
			...
			os.Setenv("POBP_TRACE_RATE_LIMIT", tt.in)
			res := newRateLimiter()
	...
	t.Run("sampling-rules", func(t *testing.T) {
		...
		for _, tt := range []struct {
			...
			t.Run("", func(t *testing.T) {
				os.Setenv("POBP_TRACE_SAMPLING_RULES", tt.value)
	...
	t.Run("matching", func(t *testing.T) {
		...
		for _, v := range ruleSets {
			t.Run("", func(t *testing.T) {
				...
				result := rs.apply(span)
				assert.True(result)
				assert.Equal(1.0, span.Metrics["_pobp.rule_psr"])
				assert.Equal(1.0, span.Metrics["_pobp.limit_psr"])
			})
		}
	})
	...
	t.Run("default-rate", func(t *testing.T) {
		...
		for _, rules := range ruleSets {
			...
			for _, rate := range sampleRates {
				t.Run("", func(t *testing.T) {
					assert := assert.New(t)
					os.Setenv("POBP_TRACE_SAMPLE_RATE", fmt.Sprint(rate))
					defer os.Unsetenv("POBP_TRACE_SAMPLE_RATE")
					...
					result := rs.apply(span)
					assert.True(result)
					assert.Equal(rate, span.Metrics["_pobp.rule_psr"])
					if rate > 0.0 {
						assert.Equal(1.0, span.Metrics["_pobp.limit_psr"])
					}
				})
...
func TestRulesSamplerInternals(t *testing.T) {
	...
	t.Run("zero-rate", func(t *testing.T) {
		...
		assert.Equal(0.0, span.Metrics["_pobp.rule_psr"])
		_, ok := span.Metrics["_pobp.limit_psr"]
		assert.False(ok)
	})
	...
	t.Run("limited-rate", func(t *testing.T) {
		...
		assert.Equal(1.0, span.Metrics["_pobp.rule_psr"])
		assert.Equal(0.75, span.Metrics["_pobp.limit_psr"])
	})
}
