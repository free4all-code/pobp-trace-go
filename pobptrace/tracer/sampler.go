...
// Sample returns true if the given span should be sampled.
func (r *rateSampler) Sample(spn pobptrace.Span) bool {
	if r.rate == 1 {
		// fast path
		return true
	}
...
// If no match is found, and the POBP_TRACE_SAMPLE_RATE environment variable
...
// limit can be defined using the POBP_TRACE_RATE_LIMIT environment variable.
...
// samplingRulesFromEnv parses sampling rules from the POBP_TRACE_SAMPLING_RULES
// environment variable.
func samplingRulesFromEnv() ([]SamplingRule, error) {
	rulesFromEnv := os.Getenv("POBP_TRACE_SAMPLING_RULES")
	if rulesFromEnv == "" {
		return nil, nil
	}
	jsonRules := []struct {
...
// globalSampleRate returns the sampling rate found in the POBP_TRACE_SAMPLE_RATE environment variable.
// If it is invalid or not within the 0-1 range, NaN is returned.
func globalSampleRate() float64 {
	defaultRate := math.NaN()
	v := os.Getenv("POBP_TRACE_SAMPLE_RATE")
	if v == "" {
		return defaultRate
	}
	r, err := strconv.ParseFloat(v, 64)
	if err != nil {
		log.Warn("ignoring POBP_TRACE_SAMPLE_RATE: error: %v", err)
		return defaultRate
	}
	if r >= 0.0 && r <= 1.0 {
		return r
	}
	log.Warn("ignoring POBP_TRACE_SAMPLE_RATE: out of range %f", r)
	return defaultRate
}

// defaultRateLimit specifies the default trace rate limit used when POBP_TRACE_RATE_LIMIT is not set.
...
// This defaults to 100.0. The POBP_TRACE_RATE_LIMIT environment variable may override the default.
func newRateLimiter() *rateLimiter {
	limit := defaultRateLimit
	v := os.Getenv("POBP_TRACE_RATE_LIMIT")
	if v != "" {
		l, err := strconv.ParseFloat(v, 64)
		if err != nil {
			log.Warn("using default rate limit because POBP_TRACE_RATE_LIMIT is invalid: %v", err)
		} else if l < 0.0 {
			log.Warn("using default rate limit because POBP_TRACE_RATE_LIMIT is negative: %f", l)
		} else {
...
// set using POBP_TRACE_SAMPLE_RATE, then it returns false and the span is not
// modified.
...
// limit returns the rate limit set in the rules sampler, controlled by POBP_TRACE_RATE_LIMIT, and
