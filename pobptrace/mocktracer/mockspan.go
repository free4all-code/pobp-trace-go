...
package mocktracer // import "git.proto.group/protoobp/pobp-trace-go/pobptrace/mocktracer"
...
var _ pobptrace.Span = (*mockspan)(nil)
...
type Span interface {
	...
	Context() pobptrace.SpanContext

	// Stringer allows pretty-printing the span's fields for debugging.
	fmt.Stringer
}

func newSpan(t *mocktracer, operationName string, cfg *pobptrace.StartSpanConfig) *mockspan {
	if cfg.Tags == nil {
...
func (s *mockspan) Finish(opts ...pobptrace.FinishOption) {
	var cfg pobptrace.FinishConfig
	for _, fn := range opts {
		fn(&cfg)
	}
	var t time.Time
...
func (s *mockspan) Context() pobptrace.SpanContext { return s.context }
