...
var prefixMsg = fmt.Sprintf("POBP Tracer %s", version.Tag)

var (
	mu     sync.RWMutex   // guards below fields
	level                 = LevelWarn
	logger pobptrace.Logger = &defaultLogger{l: log.New(os.Stderr, "", log.LstdFlags)}
)
...
func init() {
	if v := os.Getenv("POBP_LOGGING_RATE"); v != "" {
		if sec, err := strconv.ParseUint(v, 10, 64); err != nil {
			Warn("Invalid value for POBP_LOGGING_RATE: %v", err)
		} else {
...
// default is once per minute or once every POBP_LOGGING_RATE number of seconds.
...
// Log implements pobptrace.Logger.
