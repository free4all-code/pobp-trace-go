...
type Client struct {
	...
	// POBP_INSTRUMENTATION_TELEMETRY_ENABLED is set to 0 or false
	Disabled bool
...
func (c *Client) Start(integrations []Integration, configuration []Configuration) {
	c.mu.Lock()
	defer c.mu.Unlock()
	enabled := os.Getenv("POBP_INSTRUMENTATION_TELEMETRY_ENABLED")
	...
	c.Service = fromEnvOrDefault("POBP_SERVICE", c.Service)
	if len(c.Service) == 0 {
	...
	c.Env = fromEnvOrDefault("POBP_ENV", c.Env)
	c.Version = fromEnvOrDefault("POBP_VERSION", c.Version)

	c.APIKey = fromEnvOrDefault("POBP_API_KEY", c.APIKey)
