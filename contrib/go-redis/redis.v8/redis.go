...
type params struct {
	config         *clientConfig
	additionalTags []pobptrace.StartSpanOption
}
...
func additionalTagOptions(client redis.UniversalClient) []pobptrace.StartSpanOption {
	additionalTags := []pobptrace.StartSpanOption{}
...
func (ddh *datadogHook) BeforeProcess(ctx context.Context, cmd redis.Cmder) (context.Context, error) {
	...
	p := ddh.params
	opts := make([]pobptrace.StartSpanOption, 0, 4+1+len(ddh.additionalTags)+1) // 4 options below + redis.raw_command + ddh.additionalTags + analyticsRate
	opts = append(opts,
...
func (ddh *datadogHook) AfterProcess(ctx context.Context, cmd redis.Cmder) error {
	...
	var finishOpts []pobptrace.FinishOption
	errRedis := cmd.Err()
...
func (ddh *datadogHook) BeforeProcessPipeline(ctx context.Context, cmds []redis.Cmder) (context.Context, error) {
	...
	length := strings.Count(raw, " ")
	p := ddh.params
	opts := make([]pobptrace.StartSpanOption, 0, 5+1+len(ddh.additionalTags)+1) // 5 options below + redis.raw_command + ddh.additionalTags + analyticsRate
	opts = append(opts,
		tracer.SpanType(ext.SpanTypeRedis),
		tracer.ServiceName(p.config.serviceName),
		tracer.ResourceName(raw[:strings.IndexByte(raw, ' ')]),
