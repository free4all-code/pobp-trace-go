...
import (
	...
	pappsec "git.proto.group/protoobp/pobp-trace-go/appsec"
...
func TestWithHeaderTags(t *testing.T) {
	...
	r.Header.Set("x-protoobp-header", "value")
	mux.ServeHTTP(httptest.NewRecorder(), r)

	spans := mt.FinishedSpans()
	assert.Equal("header-value", spans[0].Tags()["http.request.headers.Header"])
	assert.NotContains(spans[0].Tags(), "http.headers.X-ProtoOBP-Header")
}
