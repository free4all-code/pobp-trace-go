...
package gocql // import "git.proto.group/protoobp/pobp-trace-go/contrib/gocql/gocql"
...
func (tq *Query) newChildSpan(ctx context.Context) pobptrace.Span {
	p := tq.params
...
func (tq *Query) finishSpan(span pobptrace.Span, err error) {
...
// newChildSpan creates a new span from the params and the context.
func (tb *Batch) newChildSpan(ctx context.Context) pobptrace.Span {
	p := tb.params
	opts := []pobptrace.StartSpanOption{
		tracer.SpanType(ext.SpanTypeCassandra),
		tracer.ServiceName(p.config.serviceName),
...
func (tb *Batch) finishSpan(span pobptrace.Span, err error) {
