...
) (pobptrace.Span, context.Context, error) {
...
func setSpanTargetFromPeer(span pobptrace.Span, p peer.Peer) {
...
func injectSpanIntoContext(ctx context.Context) context.Context {
	...
	if err := tracer.Inject(span.Context(), grpcutil.MDCarrier(md)); err != nil {
		...
		grpclog.Warningf("pobptrace: failed to inject the span context into the gRPC metadata: %v", err)
	}
