...
package sarama // import "git.proto.group/protoobp/pobp-trace-go/contrib/Shopify/sarama"
...
func (p *syncProducer) SendMessages(msgs []*sarama.ProducerMessage) error {
	...
	spans := make([]pobptrace.Span, len(msgs))
...
func WrapAsyncProducer(saramaConfig *sarama.Config, p sarama.AsyncProducer, opts ...Option) sarama.AsyncProducer {
	...
	go func() {
		spans := make(map[uint64]pobptrace.Span)
...
func startProducerSpan(cfg *config, version sarama.KafkaVersion, msg *sarama.ProducerMessage) pobptrace.Span {
...
func finishProducerSpan(span pobptrace.Span, partition int32, offset int64, err error) {
	span.SetTag("partition", partition)
	span.SetTag("offset", offset)
	span.Finish(tracer.WithError(err))
}

func getSpanContext(msg *sarama.ProducerMessage) (pobptrace.SpanContext, bool) {
	carrier := NewProducerMessageCarrier(msg)
